...
- `config.php` — **원본**(Rhymix/XE) 및 **대상**(GNUBoard) 데이터베이스 연결 정보. 실행 전에 반드시 수정하세요.
...
2. `config.php`에서 `$src_config`와 `$gn_config`에 올바른 접속 정보를 입력합니다.
3. 프로젝트 루트(또는 migration 디렉토리)에서 스크립트를 실행합니다. 변경을 가하지 않고 동작을 확인하려면 `--dry-run`을 사용하고 로그를 남기려면 `--log=파일`을 지정하세요.
